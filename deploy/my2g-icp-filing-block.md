...
适用对象：`codex.lizubin.online`、阿里云中国内地 ECS、Sub2API、Codex Desktop

## 1. 结论

当前 `codex.lizubin.online` 指向阿里云中国内地公网 IP `39.102.86.235`，但该域名尚未完成与中国内地接入商相匹配的 ICP 备案或接入备案。
...
```text
Codex Desktop
  -> DNS 解析 codex.lizubin.online
  -> TCP 连接 39.102.86.235:443
  -> TLS 握手，SNI=codex.lizubin.online
  -> POST /responses
  -> Nginx
  -> Sub2API
  -> 账号绑定的 Mihomo 出站代理
...
error sending request for url (https://codex.lizubin.online/responses)
```
...
所以“代理客户端已启动”不等于 `codex.lizubin.online` 一定通过境外节点。浏览器、`curl.exe` 和 Codex Desktop 也可能分别采用不同的系统代理、TUN 或直连路径。
...
```bat
nslookup codex.lizubin.online
```

检查 TLS 和健康接口：

```bat
curl.exe -vk https://codex.lizubin.online/health
```

检查流式接口：

```bat
curl.exe -vN https://codex.lizubin.online/responses ^
