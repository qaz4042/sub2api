...
// RequirePlatformEnabled blocks requests whose resolved group/forced platform
// has been disabled by the administrator. It must run after API key auth so the
// group is available in context. Forced routes (for example /antigravity/*)
// take precedence over the API key's group platform.
