...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	if provider, ok := s.settings.(OriginAwarePublicSettingsProvider); ok && provider.PublicSettingsOriginScopingEnabled() {
		s.serveOriginAwareIndexHTML(c, nonce, requestOrigin(c), provider)
		return
	}
...
func (s *FrontendServer) serveOriginAwareIndexHTML(c *gin.Context, nonce, origin string, provider OriginAwarePublicSettingsProvider) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), 2*time.Second)
	defer cancel()
	c.Header("Vary", "Host, X-Forwarded-Proto")

	settings, err := provider.GetPublicSettingsForInjectionForOrigin(ctx, origin)
	if err != nil {
		c.Data(http.StatusOK, "text/html; charset=utf-8", s.baseHTML)
		c.Abort()
		return
	...
	content := replaceNoncePlaceholder(s.injectSettings(settingsJSON), nonce)
	c.Header("Cache-Control", "no-cache")
	c.Data(http.StatusOK, "text/html; charset=utf-8", content)
	c.Abort()
}
