export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      apiTest: {
        description: '使用以下 HTTP 请求验证 API 密钥和接口连通性，也可作为业务系统对接示例。',
        openaiDescription: '使用 OpenAI 兼容接口验证 API 密钥和接口连通性，也可作为业务系统对接示例。',
        geminiDescription: '使用 Gemini v1beta 接口验证 API 密钥和接口连通性，也可作为业务系统对接示例。',
        antigravityDescription:
          'Antigravity 分组支持 Claude 与 Gemini 两类接口，以下示例可分别验证两条路径。',
        listModelsHint: '先用模型列表接口验证密钥是否可用。',
        requestHint: '复制后可直接在终端运行；业务系统对接时保留 Authorization Bearer 认证方式。',
        quickTestTitle: '快速测试',
        quickTestDescription: '选择协议和模型后，直接用当前 API 密钥发起一次真实请求。',
        protocolLabel: '协议',
        modelLabel: '模型',
        promptLabel: '测试内容',
        promptPlaceholder: '输入要发送的测试内容',
        defaultPrompt: '请用一句话回复：连接测试成功。',
        ready: '准备开始测试',
        testing: '测试中...',
        completed: '测试完成',
        start: '开始测试',
  ...
  usage: {
    ...
    apiKeyRanking: {
      title: 'API Key 排行榜',
      subtitle: '按所选时间范围内的实际消费进行全站排名',
      participants: '共 {count} 个 Key',
      myBestRank: '我的最高排名',
      outranked: '超过 {percent}% 的 Key',
      ...
      unnamedKey: '未命名 Key',
      notRanked: '暂未上榜',
      notRankedHint: '当前时间范围内产生用量后，这里会显示你的全站排名。',
      noData: '当前时间范围内暂无排行数据',
      loadFailed: '排行榜加载失败',
      loadFailedHint: '请确认后端服务已更新并重新启动，然后重试。',
      privacyHint: '为保护隐私，其他用户的 API Key 名称会脱敏显示，Key ID 和身份信息不会公开。',
    },
    errors: {
  ...
  redeem: {
    ...
    recentActivity: '最近活动',
    historyWillAppear: '您的余额或权益变更记录将显示在这里',
    balanceAddedPayment: '余额充值（在线支付）',
  ...
  admin: {
    ...
    backup: {
      ...
      actions: {
        download: '下载',
        restore: '恢复',
        downloadPasswordPrompt: '请输入管理员密码以确认下载备份',
    ...
    accounts: {
      ...
      dataExportPasswordPlaceholder: '输入当前管理员密码',
      dataExportPasswordRequired: '请输入管理员密码',
      ...
      reAuthorize: '重新授权',
      refreshToken: '刷新令牌',
      refreshSubscription: '刷新套餐',
      refreshSubscriptionSuccess: '套餐信息已刷新',
      refreshSubscriptionFailed: '刷新套餐信息失败',
      noAccountsYet: '暂无账号',
    ...
    ops: {
      ...
      healthHelp: '基于错误率、首 Token 延迟和基础设施状态的当前窗口体验风险参考。低样本时分数可能波动较大。',
      healthyStatus: '健康',
      riskyStatus: '风险',
      observingStatus: '观察',
      idleStatus: '待机',
      ...
      diagnosis: {
        ...
        idle: '系统当前处于待机状态',
        idleImpact: '无活跃流量',
        lowSample: '当前样本较少 ({count} 个请求)',
        lowSampleImpact: '健康分容易被单个慢请求放大，应结合趋势判断',
        lowSampleAction: '继续观察 10-30 分钟，或切到更长时间窗口确认',
        ...
        ttftHighImpact: '用户感知时长增加，通常与上游响应、请求体大小或账号链路有关',
        ttftHighAction: '优先按模型和账号维度查看慢请求，再判断是否需要调整调度',
        upstreamExperienceHigh: '上游体验延迟偏高 ({percentile}: {ttft}ms)',
        upstreamExperienceHighImpact: '当前更像上游响应慢，不等同于代码或机器故障',
        upstreamExperienceHighAction: '检查慢请求的模型、账号和请求体大小，必要时临时降权慢账号',
        ...
        healthCritical: '当前窗口体验风险较高 ({score})',
        healthCriticalImpact: '可能是错误率、上游延迟或基础设施异常导致，需要结合上方诊断项判断',
        healthCriticalAction: '先确认主因，再按错误、上游账号或基础设施分别处理',
        healthLow: '当前窗口体验风险偏高 ({score})',
        healthLowImpact: '可能存在轻度波动，建议关注错误率、首 Token 和上游账号表现',
        healthLowAction: '观察趋势并按主因排查，不建议仅凭单次分数判断系统故障',
        healthy: '所有系统指标正常',
    ...
    settings: {
      ...
      features: {
        platformAccess: {
          title: '接口平台',
          description: '统一控制对外开放的平台接口和首页平台展示。关闭后，对应平台的网关请求将被拒绝，首页也不再显示。',
          openai: 'OpenAI / Codex',
          openaiHint: '当前核心平台，始终启用。',
          anthropic: 'Anthropic / Claude',
          gemini: 'Google Gemini',
          antigravity: 'Antigravity',
          disabledHint: '默认关闭；开启后对应接口和首页展示同时生效。',
        },
        channelMonitor: {
          title: '渠道监控',
      ...
      site: {
        ...
        apiBaseUrl: 'API 端点地址',
        apiBaseUrlHint: '用于"使用密钥"等客户端配置，留空则使用当前站点地址',
        apiBaseUrlPlaceholder: 'https://api.example.com',
        ccsImportBaseUrl: 'CCS 导入端点地址',
        ccsImportBaseUrlHint: '仅用于 API Keys 页面的"导入 CCS"内容。留空默认使用 IP 证书直连地址 https://152.32.190.110；如需回退正式域名可在这里填写。',
        ...
        contactMethods: {
          title: '客服联系方式',
          description: '配置一个或多个客服入口，用户菜单会按列表展示为可点击入口',
          itemLabel: '联系方式 #{n}',
          type: '类型',
          typeLink: '链接',
          typeText: '文本',
          ...
          url: '跳转链接',
          add: '添加联系方式',
          valuePlaceholderLink: '在线客服',
          valuePlaceholderText: 'QQ: 123456789',
          urlPlaceholderText: '文本联系方式可留空',
        },
        docUrl: '文档链接',
