                    ...
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div class="flex min-w-0 items-center gap-3">
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                          {{ t("admin.settings.site.contactMethods.itemLabel", { n: index + 1 }) }}
                        </span>
                      ...
                      <div class="flex flex-shrink-0 items-center gap-1">
                        <button
                          ...
                          :disabled="index === 0"
                          :aria-label="t('admin.settings.site.contactMethods.moveUp')"
                        ...
                        <button
                          ...
                          :aria-label="t('admin.settings.site.contactMethods.moveDown')"
                          @click="moveContactMethod(index, 1)"
                        >
                        ...
                        <button
                          type="button"
                          class="rounded p-1 text-red-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20"
                          :aria-label="t('admin.settings.site.contactMethods.remove')"
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <select v-model="method.type" class="input text-sm">
                          <option v-for="option in contactMethodTypeOptions" :key="option.value" :value="option.value">
                            {{ option.label }}
                          </option>
                        </select>
                      </div>
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "Kaka Codex") +
                        " 100 " +
...
const form = reactive<SettingsForm>({
  ...
  site_name: "Kaka Codex",
  site_logo: "",
  site_subtitle: "AI 开发工具与交流社区",
  api_base_url: "",
  ccs_import_base_url: "",
  contact_info: "",
...
function createContactMethod(
  ...
  return {
    ...
    sort: Number.isFinite(overrides.sort) ? Number(overrides.sort) : form.contact_methods.length,
  };
}

function addContactMethod(): void {
  form.contact_methods.push(createContactMethod());
  normalizeContactMethodSort();
}

function removeContactMethod(index: number): void {
  form.contact_methods.splice(index, 1);
  normalizeContactMethodSort();
}

function moveContactMethod(index: number, direction: -1 | 1): void {
  const targetIndex = index + direction;
  if (targetIndex < 0 || targetIndex >= form.contact_methods.length) return;
  const current = form.contact_methods[index];
  form.contact_methods[index] = form.contact_methods[targetIndex];
  form.contact_methods[targetIndex] = current;
  normalizeContactMethodSort();
}

function normalizeContactMethodSort(): void {
...
function normalizeContactMethods(
  ...
  return source
    .map((method, index) => createContactMethod({
      ...method,
