...
const siteName = computed(() => appStore.siteName.trim() || DEFAULT_SITE_NAME)
...
export XAI_API_KEY="${apiKey}"`
  ...
  const configSiteName = siteName.value.replace(/[\r\n]+/g, ' ')
  const tomlSiteName = escapeTomlBasicString(configSiteName)
  const modelsListUrl = `${baseUrl.replace(/\/+$/, '')}/models`
  const configContent = `# Grok Build CLI → ${configSiteName} Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for ${configSiteName} Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches ${configSiteName}).
...
description = "Grok 4.5 via ${tomlSiteName} (Responses)"
...
# Imagine tools: model IDs go to ${configSiteName} media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → ${configSiteName} Grok group
...
# Grok/${configSiteName} path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at the site's OpenAI-compatible Responses/Chat endpoints.
