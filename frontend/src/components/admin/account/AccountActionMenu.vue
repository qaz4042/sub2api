      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAIOAuth && !isShadow" @click="$emit('refresh-subscription', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-cyan-600 hover:bg-gray-100 dark:hover:bg-dark-700">
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'refresh-subscription', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
