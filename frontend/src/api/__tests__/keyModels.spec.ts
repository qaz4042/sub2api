...
describe('key models', () => {
  it('uses the key credential and preserves discovered models without a static whitelist', async () => {
    ...
    expect(await fetchKeyModels('sk-test', 'openai', signal)).toEqual([
  ...
  it('uses same-origin Gemini and Antigravity endpoints', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ data: [] }) })
    vi.stubGlobal('fetch', fetchMock)
    await fetchKeyModels('key', 'gemini')
    expect(fetchMock.mock.calls[0][0]).toBe('/v1/models')
    await fetchKeyModels('key', 'antigravity')
    expect(fetchMock.mock.calls[1][0]).toBe('/antigravity/v1/models')
  })
